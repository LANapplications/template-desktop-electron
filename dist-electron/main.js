...
import require$$1$4, { dialog, BrowserWindow, Menu, safeStorage, app, shell, ipcMain } from "electron";
import { fileURLToPath } from "node:url";
import path$o, { join } from "node:path";
...
import { existsSync, rmSync, readFileSync as readFileSync$1, writeFileSync as writeFileSync$1 } from "node:fs";
...
path$o.join(process.env.APP_ROOT, "dist-electron");
...
function requireFile$1() {
  ...
  function readFileSyncFromEnd(filePath2, bytesCount) {
    ...
    const fd = fs2.openSync(filePath2, "r");
...
function requireFileRegistry() {
  ...
  class FileRegistry extends EventEmitter {
    ...
    provide({ filePath: filePath2, writeOptions = {}, writeAsync = false }) {
      let file2;
      try {
        filePath2 = path2.resolve(filePath2);
    ...
    testFileWriting({ filePath: filePath2, writeOptions }) {
      ...
      fs2.writeFileSync(filePath2, "", { flag: "a", mode: writeOptions.mode });
    }
  }
...
function requireRemote() {
  ...
  function remoteTransportFactory(logger) {
    ...
    function transport(message) {
      ...
      request2.on("error", (error2) => transport.processErrorFn({
        error: error2,
        logger,
        message,
        request: request2,
...
function setupLogger() {
  ...
  try {
    const logsDir = path$o.dirname(log.transports.file.getFile().path);
    const files = require$$1$1.readdirSync(logsDir).filter((f) => f.startsWith("main"));
    if (files.length > 5) {
      const sorted = files.map((f) => ({
        name: f,
        time: require$$1$1.statSync(path$o.join(logsDir, f)).mtime.getTime()
      })).sort((a, b) => b.time - a.time);
      sorted.slice(5).forEach((f) => {
        require$$1$1.unlinkSync(path$o.join(logsDir, f.name));
      });
    }
  } catch (e) {
...
function rmkids(p, options2, cb) {
  ...
  options2.readdir(p, (er, files) => {
    ...
    files.forEach((f) => {
      rimraf$1(path$j.join(p, f), options2, (er2) => {
        if (errState) {
          return;
        }
...
function rmkidsSync(p, options2) {
  assert(p);
  assert(options2);
  options2.readdirSync(p).forEach((f) => rimrafSync(path$j.join(p, f), options2));
  if (isWindows) {
...
class HttpExecutor {
  ...
  doApiRequest(options2, cancellationToken, requestProcessor, redirectCount = 0) {
    if (debug$2.enabled) {
      const { headers: _headers, auth: _auth, ...safeOptions } = options2;
      debug$2(`Request: ${safeStringifyJson(safeOptions)}`);
    }
    return cancellationToken.createPromise((resolve, reject, onCancel) => {
      const request2 = this.createRequest(options2, (response) => {
        try {
      ...
      this.addErrorAndTimeoutHandlers(request2, reject, options2.timeout);
      this.addRedirectHandlers(request2, options2, reject, redirectCount, (options3) => {
  ...
  addRedirectHandlers(request2, options2, reject, redirectCount, handler) {
  }
...
`));
  ...
  doDownload(requestOptions, options2, redirectCount) {
    ...
    this.addErrorAndTimeoutHandlers(request2, options2.callback, requestOptions.timeout);
    this.addRedirectHandlers(request2, requestOptions, options2.callback, redirectCount, (requestOptions2) => {
...
function makeSnippet$1(mark, options2) {
  ...
  for (let i = 1; i <= options2.linesBefore; i++) {
    ...
    result = common$4.repeat(" ", options2.indent) + padStart((mark.line - i + 1).toString(), lineNoLength) + " | " + line2.str + "\n" + result;
  }
...
function loadDocuments(input, options2) {
  ...
  const state = new State$1(input, options2);
...
let SemVer$d = class SemVer {
  constructor(version2, options2) {
    ...
    const m = version2.trim().match(options2.loose ? re$2[t$1.LOOSE] : re$2[t$1.FULL]);
    if (!m) {
  ...
  inc(release, identifier, identifierBase) {
    if (release.startsWith("pre")) {
      ...
      if (identifier) {
        const match = `-${identifier}`.match(this.options.loose ? re$2[t$1.PRERELEASELOOSE] : re$2[t$1.PRERELEASE]);
...
const parse$7 = (version2, options2, throwErrors = false) => {
...
const valid$2 = (version2, options2) => {
  const v = parse$6(version2, options2);
  return v ? v.version : null;
};
var valid_1 = valid$2;
const parse$5 = parse_1;
const clean$1 = (version2, options2) => {
  const s = parse$5(version2.trim().replace(/^[=v]+/, ""), options2);
...
const prerelease$1 = (version2, options2) => {
  const parsed = parse$3(version2, options2);
...
const coerce$1 = (version2, options2) => {
  ...
  let match = null;
  if (!options2.rtl) {
    match = version2.match(options2.includePrerelease ? re$1[t.COERCEFULL] : re$1[t.COERCE]);
  } else {
    const coerceRtlRegex = options2.includePrerelease ? re$1[t.COERCERTLFULL] : re$1[t.COERCERTL];
    let next;
  ...
  return parse$2(`${major2}.${minor2}.${patch2}${prerelease2}${build}`, options2);
};
...
(function(exports) {
  ...
  class ElectronHttpExecutor extends builder_util_runtime_12.HttpExecutor {
    ...
    createRequest(options2, callback) {
      ...
      const request2 = require$$1$4.net.request({
        ...options2,
...
class PrivateGitHubProvider extends GitHubProvider_1$1.BaseGitHubProvider {
  ...
  async getLatestVersionInfo(cancellationToken) {
    ...
    try {
      ...
      if (allowPrerelease) {
        const candidates = version2.filter((it) => !it.draft);
...
function setupUpdater() {
  ...
  main$2.autoUpdater.on("update-available", () => {
    dialog.showMessageBox({
      type: "info",
      title: "Actualización disponible",
      message: "Se encontró una nueva versión. Se descargará automáticamente en segundo plano."
    });
  });
  main$2.autoUpdater.on("download-progress", (progress) => {
    console.log(`Descargando update: ${Math.round(progress.percent)}%`);
  });
  main$2.autoUpdater.on("update-downloaded", () => {
    dialog.showMessageBox({
      type: "info",
...
function createWindow() {
  ...
  else win.loadFile(path$o.join(RENDERER_DIST, "index.html"));
...
function configDotenv(options2) {
  ...
  if (debug2 || !quiet) {
    ...
    for (const filePath2 of optionPaths) {
      try {
        const relative = path.relative(process.cwd(), filePath2);
...
function assertAuthConfig() {
  const missing = ["domain", "clientId", "audience"].filter(
    (key) => !authConfig[key]
  );
  if (missing.length > 0) {
    const vars = missing.map((key) => ({ domain: "AUTH0_DOMAIN", clientId: "AUTH0_CLIENT_ID", audience: "AUTH0_AUDIENCE" })[key]).join(", ");
    throw new Error(`Auth0 no está configurado. Completá ${vars} en el archivo .env`);
  }
}
const filePath = () => join(app.getPath("userData"), "auth.json");
const tokenStore = {
  save(tokens) {
    const json2 = JSON.stringify(tokens);
    const data = safeStorage.isEncryptionAvailable() ? safeStorage.encryptString(json2) : Buffer.from(json2, "utf8");
    writeFileSync$1(filePath(), data);
  },
  load() {
    const path2 = filePath();
    if (!existsSync(path2)) return null;
    try {
      const data = readFileSync$1(path2);
      const json2 = safeStorage.isEncryptionAvailable() ? safeStorage.decryptString(data) : data.toString("utf8");
...
function runBrowserFlow(targetUrl, redirectUri) {
  ...
  return new Promise((resolve, reject) => {
    ...
    timer = setTimeout(() => {
      const p = pending;
      teardown();
      p == null ? void 0 : p.reject(new Error("Se agotó el tiempo de espera del login"));
    }, 5 * 60 * 1e3);
    ...
    const srv = http.createServer((req, res) => {
      ...
      const p = pending;
      teardown();
      p == null ? void 0 : p.resolve(fullUrl);
      if (win && !win.isDestroyed()) win.focus();
    });
    srv.on("error", (err) => {
      const p = pending;
      teardown();
      p == null ? void 0 : p.reject(
        new Error(
...
function createPkce() {
  const verifier = base64url(crypto$1.randomBytes(32));
  const challenge = base64url(
    crypto$1.createHash("sha256").update(verifier).digest()
  );
  return { verifier, challenge };
}
function decodeJwt(token) {
  const payload = token.split(".")[1] ?? "";
  const json2 = Buffer.from(payload, "base64url").toString("utf8");
  return JSON.parse(json2);
}
async function authorize(screenHint) {
  assertAuthConfig();
  const { verifier, challenge } = createPkce();
  const state = base64url(crypto$1.randomBytes(16));
  ...
  if (error2) {
    if (error2 === "access_denied") throw new Error("AUTH_CANCELLED");
    throw new Error(url.searchParams.get("error_description") ?? error2);
  }
...
async function request(method, path2, body) {
  ...
  const res = await fetch(`${apiConfig.baseUrl}${path2}`, {
    method,
    headers: {
      "Content-Type": "application/json",
  ...
  if (!res.ok) {
    ...
    try {
      ...
      if (data == null ? void 0 : data.message) message = data.message;
    } catch {
    }
...
function registerClientIPC() {
  ...
  ipcMain.handle(
    "createClient",
    (_event, data) => clientApi.create(data)
  );
  ipcMain.handle(
    "updateClient",
    (_event, id, data) => clientApi.update(id, data)
  );
  ipcMain.handle(
    "deleteClient",
    (_event, id) => clientApi.remove(id)
  );
}
setupLogger();
setupUpdater();
