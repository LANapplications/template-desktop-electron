...
import path$n, { join } from "node:path";
...
path$n.join(process.env.APP_ROOT, "dist-electron");
...
function setupLogger() {
  ...
  try {
    const logsDir = path$n.dirname(log.transports.file.getFile().path);
    const files = require$$1$1.readdirSync(logsDir).filter((f) => f.startsWith("main"));
    if (files.length > 5) {
      const sorted = files.map((f) => ({
        name: f,
        time: require$$1$1.statSync(path$n.join(logsDir, f)).mtime.getTime()
      })).sort((a, b) => b.time - a.time);
      sorted.slice(5).forEach((f) => {
        require$$1$1.unlinkSync(path$n.join(logsDir, f.name));
      });
    }
  } catch (e) {
...
function createWindow() {
  ...
  else win.loadFile(path$n.join(RENDERER_DIST, "index.html"));
  Menu.setApplicationMenu(null);
  return win;
}
const authConfig = {
  domain: "dev-mumxvnpryflz2ufj.us.auth0.com",
  clientId: "2zdKw3yaY7ZWVpiuWTOe7o60Kw3CJVHH",
  audience: "https://api.wearelan.com/",
  ...
  callbackUrl: "http://127.0.0.1:41730/callback"
};
const apiConfig = {
...
async function authorize(screenHint) {
  ...
  params.set("prompt", authConfig.prompt);
